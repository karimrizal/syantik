...
class VerifikasiRekomendasiBPS extends Controller
{
    public function index()
    {
        $rekomendasis = rekomendasi::where('idkab', Auth::user()->idkab )->get();
        $kompromins = kompromin::where('idkab', Auth::user()->idkab )->get();
        return view('verifikasi_rekomendasi_bps',compact('rekomendasis', 'kompromins'))->with('k', (request()->input('page', 1) - 1) * 5)->with('l', (request()->input('page', 1) - 1) * 5);
     ...
     public function edit_kompromin_v(Request $request, $id)
    {
      kompromin::updateOrCreate(
       ...
       [
        ...
        'status_k_bps' => $request->status_k_bps,
        'komentar_k_bps' => $request->komentar_k_bps,
        'petugas_bps' => $request->petugas_bps,
        'tanggal_k_bps' => Carbon::now()
       ]
      );
