                        ...
                        @if( (Auth::user()->role == 'Admin OPD')|| (Auth::user()->role == 'Admin BPS Kabupaten') )  <li><a href="user_opd" class=""><i class="lnr lnr-user"></i> <span>Kelola Pengguna</span></a></li> @else @endif
							...
							<div id="subPages2" class="collapse in" aria-expanded="false" style="">
								<ul class="nav">
									...
									@if( (Auth::user()->role == 'Operator OPD') )<li><a href="verifikasi_data_opd" class="active">Verifikasi Data</a></li> @else<li><a href="verifikasi_data" class="active">Verifikasi Data</a></li>@endif
               ...
               @foreach($tabels_opd as $user)
                  ...
                  @else <td> <form action = "{{ route('postsss.show4',$user->id) }}" method="GET">
                   ...
                   <td> @if($user->status_bps =='Tampil') <center> <i class="fa fa-check" style="color:green"></i></center>
                      @elseif($user->status_bps =='Tidak Tampil')<center> <i class="fa fa-close" style="color:red"></i></center>@else @endif<br> @if($user->status_opd == 'Tidak Tampil' )  @elseif($user->status_bps == 'Tidak Tampil' )<a href="" id="editCompany2" data-toggle="modal" data-target='#practice_modal2' data-id="{{ $user->id }}">lihat catatan</a>	
                     @elseif(is_null($user->status_bps) && $user->status_opd == 'Tampil' && $user->status == 'Tampil') Belum Diverifikasi @elseif(is_null($user->status_opd) || is_null($user->status)) Menunggu Verifikasi Walidata @else</td> @endif 
               ...
               @foreach($tabels as $user)
                ...
                <td> {{ $user->organisasi }}</td>
                  ...
                  @else <td> <form action = "{{ route('postsss.show',$user->id) }}" method="GET">
                   ...
                   <td> @if($user->status_bps =='Tampil') <center> <i class="fa fa-check" style="color:green"></i></center>
                      @elseif($user->status_bps =='Tidak Tampil')<center> <i class="fa fa-close" style="color:red"></i></center>@else @endif<br> @if($user->status_opd == 'Tidak Tampil' )  @elseif($user->status_bps == 'Tidak Tampil' )<a href="" id="editCompany2" data-toggle="modal" data-target='#practice_modal2' data-id="{{ $user->id }}">lihat catatan</a>	
                     @elseif(is_null($user->status_bps) && $user->status == 'Tampil')Belum Diverifikasi @elseif(is_null($user->status)) Menunggu Verifikasi Walidata @else</td> @endif 
                ...
                <td> {{ $user->organisasi }}</td>
                  ...
                  <td> @if($user->status =='Tampil') <center> <i class="fa fa-check" style="color:green"></i></center>
                     ...
                     @elseif(is_null($user->status) && $user->status_opd == 'Tampil')Belum Diverifikasi @elseif(is_null($user->status_opd)) Menunggu Verifikasi OPD @else</td> @endif </td>
                       ...
                       @if($user->status_opd == 'Tidak Tampil' || $user->status == 'Tidak Tampil' )
                    <td></td>
                     @elseif(is_null($user->status) || $user->status == 'Tidak Tampil'  )
                    <td> Menunggu Verifikasi Walidata</td>
                    
                  
                    
                    @elseif (is_null($user->status_bps) && $user->status == 'Tampil' )
                        ...
                        <div class="modal-dialog">
                           <form id="companydata4">
                               ...
                               <div class="modal-body">
                                    ...
                                    <input type="text" name="perbaikan_opd" id="perbaikan_opd" value="" class="form-control" readonly>
                                ...
                                <div class="modal-body">
                                    ...
                                    <input type="text" name="catatan_perbaikan" id="catatan_perbaikan" value="" class="form-control" readonly>
...
$.ajaxSetup({
    ...
    var id = $(this).data('id');
    console.log(id)
    $.get('ket_wali/' + id + '/edit_ketWali', function (data) {
         $('#userCrudModal').html("Edit category");
         $('#submit').val("Edit category");
         $('#practice_modal').modal('show');
         $('#color_id').val(data.data.id);
         $('#alasan_tampil_walidata').val(data.data.alasan_tampil_walidata);
    ...
    var id = $(this).data('id');
    console.log(id)
    $.get('ket_bps/' + id + '/edit_ketBps', function (data) {
         $('#userCrudModal').html("Edit category");
         $('#submit').val("Edit category");
         $('#practice_modal2').modal('show');
         $('#color_id2').val(data.data.id);
         $('#alasan_tampil_bps').val(data.data.alasan_tampil_bps);
    ...
    var id = $(this).data('id');
    console.log(id)
    $.get('ket_opd/' + id + '/edit_ketOpd', function (data) {
         $('#userCrudModal').html("Edit category");
         $('#submit').val("Edit category");
         $('#practice_modal3').modal('show');
         $('#color_id3').val(data.data.id);
         $('#alasan_tampil_opd').val(data.data.alasan_tampil_opd);
...
$('body').on('click', '#editCompany4', function (event) {
    ...
    var id = $(this).data('id');
    console.log(id)
    $.get('ket_perbaikan/' + id + '/edit_ketPerbaikan', function (data) {
         $('#userCrudModal').html("Edit category");
         $('#submit').val("Edit category");
