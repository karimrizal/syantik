...
<br>
    ...
    <form action="{{ url('https://webapps.bps.go.id/sultra/syantik/verifikasi_data') }}" method="POST">
	@csrf
	@foreach ($tabels as $result)
	
								<input type="hidden" id="id" name="user_id" value="{{ $result->id }}">
								<input type="hidden" id="petugas" name="petugas" value="{{Auth::user()->name}}">
								
								<div class="form-group">
									<label for="">Apakah data akan ditampilkan?</label><br>
									<input type="radio" name="status" id="status" value="Tampil" required>
									<label for="tampil">Tampil</label><br>
									<input type="radio" name="status" id="status" value="Tidak Tampil" required>
									<label for="tidak_tampil">Tidak Tampil</label>
								</div>
								<div class="form-group">
									<label for="alasan_tampil">Alasan jika tidak tampil</label>
									<input class="form-control" type="text" name="alasan_tampil_walidata" id="alasan_tampil_walidata" value="{{ $result->alasan_tampil_walidata }}" />
								</div>
