...
<div class="card-header">    </div>
    ...
    @if (session('success'))
               ...
               <p class="page-title text-left"><a href="data_dda"><i class="lnr lnr-arrow-left-circle"></i> Kembali</a></p>
                           ...
                           <a href="{{ url('/ekspor_dda_62') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>    
					@foreach($kab as $a)				
<h3 class="panel-title" style="font-weight: 500; font-size: 20px; color: #006fcc;">Jumlah Pelanggan Listrik Menurut Unit Layanan Pelanggan (ULP) di {{ $a->kab }}, {{$year}}</h3>
@endforeach				
<br>
...
<table class="table table-bordered" id="example3">
                <thead>
                    ...
                    @foreach ($tabel_62 as $i => $member)
                    <tr>
                     
                        <th>{{ $member->kec }}</th>
                        
                         <form id="simpan" action="{{ route('dda.store62') }}" method="POST">
                             @csrf
                          ...
                          @if( $a->setuju_opd == 1)
                               ...
                               @if(is_numeric($member->t62a))
                            <td>
                                <input type="text" id="t62a" name="members[{{ $i }}][t62a]" class="form-control" value="{{number_format(($member->t62a),0, '.','')}}" disabled>
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t62a" name="members[{{ $i }}][t62a]" class="form-control" value="{{$member->t62a}}" disabled>
                            </td>
                            @endif    
                        ...
                        @elseif( $a->setuju_opd == 0)
                          ...
                          @if(is_numeric($member->t62a))
                            <td>
                                <input type="text" id="t62a" name="members[{{ $i }}][t62a]" class="form-control" value="{{number_format(($member->t62a),0, '.','')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t62a" name="members[{{ $i }}][t62a]" class="form-control" value="{{$member->t62a}}">
                            </td>
                            @endif 
  ...
  <tr> <th bgcolor="#5cb85c" style="vertical-align: middle;"> <center>ULP</center></th>
               ...
               @foreach ($tes as $member)
                    <tr>
                      ...
                      <th>{{ $member->kec }}</th>
                             ...
                             @if(is_numeric($member->t62a))
                            <td>
                                <input type="text" readonly="ture" class="form-control" value="{{number_format(($member->t62a),0, '.',' ')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" readonly="ture" class="form-control" value="{{$member->t62a}}">
                            </td>
                            @endif    
