...
<div class="card-header">    </div>
	...
	@if (session('success'))
               ...
               <p class="page-title text-left"><a href="data_dda"><i class="lnr lnr-arrow-left-circle"></i> Kembali</a></p>
                            ...
                            <a href="{{ url('/ekspor_7410') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>    
					@foreach($kab as $a)	
					@if( (Auth::user()->idkab == '7400') )
<h3 class="panel-title" style="font-weight: 500; font-size: 20px; color: #006fcc;"> Jumlah Keluarga yang Mendapatkan Bantuan PKH di Provinsi Sulawesi Tenggara Menurut Kabupaten/Kota, {{$year}} </h3>
@else
...
<table class="table table-bordered" id="example3">
                <thead>
                    ...
                    <tr>
                     ...
                     @foreach ($tabel_7410 as $i => $member)
                    <tr>
                     
                        <th>{{ $member->kec }}</th>
                        
                         <form id="simpan" action="{{ route('dda.store7410') }}" method="POST">
                             @csrf
                          ...
                          @if( $a->setuju_opd == 1)
                             ...
                             @if(is_numeric($member->t7410a))
                            <td>
                                <input type="text" id="t7410a" name="members[{{ $i }}][t7410a]" class="form-control" value="{{number_format(($member->t7410a),0, '.','')}}" disabled>
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t7410a" name="members[{{ $i }}][t7410a]" class="form-control" value="{{$member->t7410a}}" disabled>
                            </td>
                            @endif    
                         ...
                         @elseif( $a->setuju_opd == 0)  
                          ...
                          @if(is_numeric($member->t7410a))
                            <td>
                                <input type="text" id="t7410a" name="members[{{ $i }}][t7410a]" class="form-control" value="{{number_format(($member->t7410a),0, '.','')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t7410a" name="members[{{ $i }}][t7410a]" class="form-control" value="{{$member->t7410a}}">
                            </td>
                            @endif   
...
<p style="font-size: 14px;color: #8D99A8;">Sumber: Dinas Sosial</a></p>
