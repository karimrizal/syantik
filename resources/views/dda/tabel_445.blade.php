...
<div class="card-header">    </div>
 ...
 <a href="{{ url('/ekspor_dda_445') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>                                    
					@foreach($kab as $a)				
<h3 class="panel-title" style="font-weight: 500; font-size: 20px; color: #006fcc;">Jumlah Kasus Narkoba Menurut Kabupaten/Kota di {{ $a->kab }}, {{$year}}</h3>
@endforeach				
<br>
...
<table class="table table-bordered" id="example3">
                <thead>
                    ...
                    @foreach ($tabel_445 as $i => $member)
                    <tr>
                        
                        <th>{{ $member->kec }}</th>
                        
                         <form id="simpan" action="{{ route('dda.store445') }}" method="POST">
                             @csrf
                          ...
                          @if( $a->setuju_opd == 1)
                              ...
                              @if(is_numeric($member->t445a))
                            <td>
                                <input type="text" id="t445a" name="members[{{ $i }}][t445a]" class="form-control" value="{{number_format(($member->t445a),0, '.','')}}" disabled>
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t445a" name="members[{{ $i }}][t445a]" class="form-control" value="{{$member->t445a}}" disabled>
                            </td>
                            @endif    
                            ...
                            @elseif( $a->setuju_opd == 0)
                             ...
                             @if(is_numeric($member->t445a))
                            <td>
                                <input type="text" id="t445a" name="members[{{ $i }}][t445a]" class="form-control" value="{{number_format(($member->t445a),0, '.','')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t445a" name="members[{{ $i }}][t445a]" class="form-control" value="{{$member->t445a}}">
                            </td>
                            @endif   
               ...
               @foreach ($tes as $member)
                    <tr>
                      ...
                      <th>{{ $member->kec }}</th>
                            ...
                            @if(is_numeric($member->t445a))
                            <td>
                                <input type="text" readonly="true" class="form-control" value="{{number_format(($member->t445a),0, '.',' ')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" readonly="true" class="form-control" value="{{$member->t445a}}">
                            </td>
                            @endif    
