...
<div class="card-header">    </div>
	...
	@if (session('success'))
               ...
               <p class="page-title text-left"><a href="data_dda"><i class="lnr lnr-arrow-left-circle"></i> Kembali</a></p>
                           ...
                           <a href="{{ url('/ekspor_dda_443') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>          
					@foreach($kab as $a)				
<h3 class="panel-title" style="font-weight: 500; font-size: 20px; color: #006fcc;">Jumlah Tindak Pidana Khusus Menurut Jenis Kriminalitas di {{ $a->kab }}, {{$year}}</h3>
@endforeach				
<br>
...
<table class="table table-bordered" id="example3">
                <thead>
                    ...
                    @foreach ($tabel_443 as $i => $member)
                    <tr>
                        
                        <th>{{ $member->kec }}</th>
                        
                         <form id="simpan" action="{{ route('dda.store443') }}" method="POST">
                             @csrf
                          ...
                          @if( $a->setuju_opd == 1)
                              ...
                              @if(is_numeric($member->t443a))
                            <td>
                                <input type="text" id="t443a" name="members[{{ $i }}][t443a]" class="form-control" value="{{number_format(($member->t443a),0, '.','')}}" disabled>
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t443a" name="members[{{ $i }}][t443a]" class="form-control" value="{{$member->t443a}}" disabled>
                            </td>
                            @endif    
                          ...
                          @elseif( $a->setuju_opd == 0)
                           ...
                           @if(is_numeric($member->t443a))
                            <td>
                                <input type="text" id="t443a" name="members[{{ $i }}][t443a]" class="form-control" value="{{number_format(($member->t443a),0, '.','')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" id="t443a" name="members[{{ $i }}][t443a]" class="form-control" value="{{$member->t443a}}">
                            </td>
                            @endif    
               ...
               @foreach ($tes as $member)
                    <tr>
                      ...
                      <th>{{ $member->kec }}</th>
                              ...
                              @if(is_numeric($member->t443a))
                            <td>
                                <input type="text" readonly="true" class="form-control" value="{{number_format(($member->t443a),0, '.',' ')}}">
                            </td>
                           
                           @else
                              <td>
                                <input type="text" readonly="true" class="form-control" value="{{$member->t443a}}">
                            </td>
                            @endif    
