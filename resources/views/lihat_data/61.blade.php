...
<div class="card-header">    </div>
	...
	<p class="page-title text-left"><a href="{{ url()->previous() }}"><i class="lnr lnr-arrow-left-circle"></i> Kembali</a></p>
                           <br>  <a href="{{ url('/ekspor_dda_61') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>    
					@foreach($kab as $a)				
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th>{{ $member->kec }}</th>
                            ...
                            <td>
                                <center> {{number_format(($member->t61a),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                               <center> {{$member->t61a}} </center>
                            </td>
                            @endif    
                            
                             @if(is_numeric($member->t61b))
                            <td>
                                <center> {{number_format(($member->t61b),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t61b}} </center>
                            </td>
                            @endif
                            
                             @if(is_numeric($member->t61c))
                            <td>
                                <center> {{number_format(($member->t61c),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t61c}} </center>
                            </td>
                            @endif    
                            
                            @if(is_numeric($member->t61d))
                            <td>
                                <center> {{number_format(($member->t61d),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t61d}} </center>
                            </td>
                            @endif 
                            
                             @if(is_numeric($member->t61e))
                            <td>
                                <center> {{number_format(($member->t61e),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t61e}} </center>
                            </td>
                            @endif 
