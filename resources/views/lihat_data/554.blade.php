...
<div class="card-header">    </div>
	...
	<p class="page-title text-left"><a href="{{ url()->previous() }}"><i class="lnr lnr-arrow-left-circle"></i> Kembali</a></p>
                           <br>  <a href="{{ url('/ekspor_dda_554') }}"><button type="button" class="btn btn-primary" style="float: right;background-color:#006fcc;">Ekspor Excel</button></a>
<br>
<br>    
					@foreach($kab as $a)
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th>{{ $member->kec }}</th>
                            ...
                            <td>
                                <center> {{number_format(($member->t554a),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                               <center> {{$member->t554a}} </center>
                            </td>
                            @endif    
                            
                             @if(is_numeric($member->t554b))
                            <td>
                                <center> {{number_format(($member->t554b),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t554b}} </center>
                            </td>
                            @endif
                            
                             @if(is_numeric($member->t554c))
                            <td>
                                <center> {{number_format(($member->t554c),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t554c}} </center>
                            </td>
                            @endif    
                            
                            @if(is_numeric($member->t554d))
                            <td>
                                <center> {{number_format(($member->t554d),0, '.',' ')}} </center>
                            </td>
                           
                           @else
                              <td>
                                <center> {{$member->t554d}} </center>
                            </td>
                            @endif 
